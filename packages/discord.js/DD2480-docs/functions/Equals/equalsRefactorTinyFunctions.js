...
class ApplicationCommand extends Base {
  ...
  equals(appCommand, enforceOptionOrder = false) {
    ...
    return !this.equalId(appCommand)
        || !this.equalName(appCommand)
        || !this.equalType(appCommand)
        || !this.equalDescription(appCommand)
        || !this.equalVersion(appCommand)
        || !this.equalNsfw(appCommand)
        || !this.equalLength(appCommand)
        || !this.equalContext(appCommand)
        || !this.equalNameLocalization(appCommand)
        || !this.equalDescriptionLocalization(appCommand)
        || !this.equalIntegrationTypes(appCommand)
        || !this.equalOption(appCommand, enforceOptionOrder)
        || !this.equalDefaultMemberPermissions(appCommand)
  }

  equalId(appCommand) {
    return command.id && this.id === command.id
  }

  equalName(appCommand) {
    return appCommand.name === this.name
  }

  equalType(appCommand) {
    return appCommand.type && appCommand.type === this.type
  }

  equalDescription(appCommand) {
    return ('description' in appCommand && appCommand.description === this.description)
  }

  equalVersion(appCommand) {
    return ('version' in appCommand && appCommand.version === this.version)
  }

  equalNsfw(appCommand) {
    return ('nsfw' in appCommand && appCommand.nsfw === this.nsfw) 
  }

  equalLength(appCommand) {
    return (appCommand.options?.length ?? 0) === (this.options?.length ?? 0)
  }

  equalNameLocalization(appCommand) {
    return isEqual(appCommand.nameLocalizations ?? appCommand.name_localizations ?? {}, this.nameLocalizations ?? {})
  }
  equalDescriptionLocalization(appCommand) {
    return isEqual(appCommand.descriptionLocalizations ?? appCommand.description_localizations ?? {}, this.descriptionLocalizations ?? {})
  }
  equalIntegrationTypes(appCommand) {
    return isEqual(appCommand.integrationTypes ?? appCommand.integration_types ?? [], this.integrationTypes ?? [])
  }
  equalContext(appCommand) {
    return isEqual(appCommand.contexts ?? [], this.contexts ?? [])
  }

  equalOption(appCommand, enforceOptionOrder = false) {
  ...
  equalDefaultMemberPermissions(appCommand) {
    ...
    return defaultMemberPermissions === (this.defaultMemberPermissions?.bitfield ?? null)
  }
